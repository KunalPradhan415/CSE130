...
(* build: (int*int->int) * int -> Expr 
  ...
  * the first is a random function.  
  * ie: A call to rand(2,5) will give
...
let rec build (rand,depth) = 
  let x = 
    if depth = 0 then rand (0,2) else rand (2, 7) in
  match x with 
  | 0 -> buildX()  
  | 1 -> buildY()
 ...
 * you a random number in the range [2,5)  
 * (2 inclusive, and 5 exclusive).
*  Same as previous function except for the 2 new operators
*)
let rec build2 (rand,depth) =
  let x = 
    if depth = 0 then rand (0,2) else rand (2, 9) in
  match x with 
  | 0 -> buildX()  
  | 1 -> buildY()
  | 2 -> buildSine(build(rand, depth-1))
  ...
  | _ -> buildThreeMull(build(rand, depth-1), build(rand, depth-1), build(rand, depth-1))
;;
